...
(
    ...
    for(genvar i=0; i<ROWS; i++) begin
        ...
        // all instructions enter from the left of each row
        ...
        // all instructions exit from the right of each row
    ...
    {% set max_r = cells | map(attribute='coordinates.row') | max %}
    {% set max_c = cells | map(attribute='coordinates.col') | max %}
    {% for cell in cells %}
    ...
    `{{cell.cell.name}} cell_{{r}}_{{c}}_inst (
        ...
        {% if r == 0 %}
        .bulk_intercell_n_in('0),
        {% else %}
        .bulk_intercell_n_in(bulk_intercell_s2n_net[{{r-1}}][{{c}}]),
        {% endif %}
        {% if c == 0 %}
        .bulk_intercell_w_in('0),
        {% else %}
        .bulk_intercell_w_in(bulk_intercell_e2w_net[{{r}}][{{c-1}}]),
        {% endif %}
        {% if r == max_r %}
        .bulk_intercell_s_in('0),
        {% else %}
        .bulk_intercell_s_in(bulk_intercell_n2s_net[{{r+1}}][{{c}}]),
        {% endif %}
        {% if c == max_c %}
        .bulk_intercell_e_in('0),
        {% else %}
        .bulk_intercell_e_in(bulk_intercell_w2e_net[{{r}}][{{c+1}}]),
        {% endif %}
        .bulk_intercell_n_out(bulk_intercell_n2s_net[{{r}}][{{c}}]),
        .bulk_intercell_w_out(bulk_intercell_w2e_net[{{r}}][{{c}}]),
        .bulk_intercell_s_out(bulk_intercell_s2n_net[{{r}}][{{c}}]),
        .bulk_intercell_e_out(bulk_intercell_e2w_net[{{r}}][{{c}}])
    );
    {% endfor %}
endmodule
