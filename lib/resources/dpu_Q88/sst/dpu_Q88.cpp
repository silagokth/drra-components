...
void Dpu_q88::handleDPU(const DPU_Q88_PKG::DPUInstruction &instr) {
  ...
  next_timing_states[current_fsm].addEvent(
      ...
      [this] { accumulate_register = 0; });

  // Add the event handler
  fsmHandlers[current_fsm] = DPU_Q88_Operations::getDPUHandler(
      this, (DPU_Q88_PKG::DPU_MODE)instr.mode);
}
...
void Dpu_q88::handleFSM(const DPU_Q88_PKG::FSMInstruction &instr) {
...
void Dpu_q88::handleOperation(
    std::string name, std::function<int16_t(int16_t, int16_t)> operation) {
