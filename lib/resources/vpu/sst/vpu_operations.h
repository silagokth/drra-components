...
// Value written by the set-if-compare modes (seq/sne/slt/sle/sgt/sge and their
// v* / *_imm variants) when the predicate holds. rtl/alu_wrapper.sv.j2 assigns
// `'1` — all ones across WORD_BITWIDTH — which reads back as -1 once the word
// is sign-extended. Returning 1 here made every one of those modes disagree
// with RTL.
constexpr int64_t SET_TRUE = -1;
