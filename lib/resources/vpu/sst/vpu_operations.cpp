...
// vadd_acc / vsub_acc were declared in vpu_operations.h and referenced from
// createHandlers() but never defined — the element library carried two
// undefined symbols. They have no `case` in rtl/alu_wrapper.sv.j2 either, so
// implementing them here would give a mode that passes SST and reads back
// zeros from RTL. Kept fatal, symmetric with add_acc / sub_acc above.
void handle_vadd_acc(Vpu *vpu) {
  vpu->getOutput().fatal(CALL_INFO, -1, "vadd_acc is not implemented in VPU\n");
}

void handle_vsub_acc(Vpu *vpu) {
  vpu->getOutput().fatal(CALL_INFO, -1, "vsub_acc is not implemented in VPU\n");
}
...
void handle_seq(Vpu *vpu) {
  vpu->handleOperation("seq",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (size_t i = 0; i < a.size(); i++) {
                           result.push_back(a[i] == b[0] ? SET_TRUE : 0);
                         }
                         return result;
                       });
}
...
void handle_vseq(Vpu *vpu) {
  vpu->handleOperation("vseq",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (size_t i = 0; i < a.size(); i++) {
                           result.push_back(a[i] == b[i] ? SET_TRUE : 0);
                         }
                         return result;
                       });
}
...
void handle_add_imm(Vpu *vpu) {
  vpu->handleImmOperation("add_imm",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (auto &elem : a) {
                           result.push_back(add_sat(elem, b[0]));
...
void handle_sub_imm(Vpu *vpu) {
  vpu->handleImmOperation("sub_imm",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (auto &elem : a) {
                           result.push_back(add_sat(elem, -b[0]));
...
void handle_min_imm(Vpu *vpu) {
  vpu->handleImmOperation("min_imm",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (size_t i = 0; i < a.size(); i++) {
                           result.push_back(std::min(a[i], b[0]));
...
void handle_max_imm(Vpu *vpu) {
  vpu->handleImmOperation("max_imm",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (size_t i = 0; i < a.size(); i++) {
                           result.push_back(std::max(a[i], b[0]));
...
void handle_lshift_imm(Vpu *vpu) {
  vpu->handleImmOperation("lshift_imm",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (size_t i = 0; i < a.size(); i++) {
                           result.push_back(a[i] << b[0]);
...
void handle_rshift_imm(Vpu *vpu) {
  vpu->handleImmOperation("rshift_imm",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (size_t i = 0; i < a.size(); i++) {
                           result.push_back(a[i] >> b[0]);
...
void handle_seq_imm(Vpu *vpu) {
  vpu->handleImmOperation("seq_imm",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (size_t i = 0; i < a.size(); i++) {
                           result.push_back(a[i] == b[0] ? SET_TRUE : 0);
...
void handle_sne_imm(Vpu *vpu) {
  vpu->handleImmOperation("sne_imm",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (size_t i = 0; i < a.size(); i++) {
                           result.push_back(a[i] != b[0] ? SET_TRUE : 0);
...
void handle_slt_imm(Vpu *vpu) {
  vpu->handleImmOperation("slt_imm",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (size_t i = 0; i < a.size(); i++) {
                           result.push_back(a[i] < b[0] ? SET_TRUE : 0);
...
void handle_sle_imm(Vpu *vpu) {
  vpu->handleImmOperation("sle_imm",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (size_t i = 0; i < a.size(); i++) {
                           result.push_back(a[i] <= b[0] ? SET_TRUE : 0);
...
void handle_sgt_imm(Vpu *vpu) {
  vpu->handleImmOperation("sgt_imm",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (size_t i = 0; i < a.size(); i++) {
                           result.push_back(a[i] > b[0] ? SET_TRUE : 0);
...
void handle_sge_imm(Vpu *vpu) {
  vpu->handleImmOperation("sge_imm",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (size_t i = 0; i < a.size(); i++) {
                           result.push_back(a[i] >= b[0] ? SET_TRUE : 0);
...
void handle_and_imm(Vpu *vpu) {
  vpu->handleImmOperation("and_imm",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (size_t i = 0; i < a.size(); i++) {
                           result.push_back(a[i] & b[0]);
...
void handle_xor_imm(Vpu *vpu) {
  vpu->handleImmOperation("xor_imm",
                       [](std::vector<int64_t> a, std::vector<int64_t> b) {
                         std::vector<int64_t> result;
                         for (size_t i = 0; i < a.size(); i++) {
                           result.push_back(a[i] ^ b[0]);
