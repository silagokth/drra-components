...
fn get_timing_model(op: Op) -> String {
    // Same shape as every other resource in this library (dpu / io / iosram /
    // rf / swb): one `rop` describes one AGU port, so events stack and the
    // repetition / transition operators fold over them.
    //
    // The previous version kept per-port HashMaps and merged all four VPU AGUs
    // into a single expression, which is not what a rop means here.
    ...
    for instr in op.body {
        ...
        match instr.kind.as_str() {
            "vpu" => {
                // NOTE: VPU instruction does not affect the timing model
            }
            "evt" => {
                segments.push(format!("e{}", event_counter));
                event_counter += 1;
            }
            // REPX carries the high half of the REP fields: config only, no
            // timing contribution (the base REP already supplied iter/delay).
            "repx" => continue,
            "rep" => {
