  ...
  for instr in op.body {
    ...
    match instr.kind.as_str() {
      "vpu" => {
        ...
        todo!("Implement timing model for instruction vpu");
      },
      "rep" => {
      ...
      "trans" => {
        ...
        let mut delay = instr_segments.get_value("delay");
        let mut level = instr_segments.get_value("level");
        todo!("Implement timing model for instruction trans");
      },
      _ => {
...
fn reshape_instr(op: Op) -> Op {
  ...
  for instr in op.body {
    ...
    match instr.kind.as_str() {
      "vpu" => {
        let mut option = instr
            .params
            .get_value("option")
            .parse::<i64>()
            .expect("Failed to parse option as i64");
        let mut mode = instr
            .params
            .get_value("mode")
            .parse::<i64>()
            .expect("Failed to parse mode as i64");
        let mut immediate = instr
            .params
            .get_value("immediate")
            .parse::<i64>()
            .expect("Failed to parse immediate as i64");
        todo!("Implement instruction reshaping for instruction vpu");
      },
      "rep" => {
        let mut port = instr
            .params
      ...
      "trans" => {
        ...
        let mut level = instr
            ...
            .parse::<i64>()
            .expect("Failed to parse level as i64");
        todo!("Implement instruction reshaping for instruction trans");
      },
      _ => {
