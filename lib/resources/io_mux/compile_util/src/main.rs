...
include!(concat!(env!("OUT_DIR"), "/isa_config.rs"));
...
fn get_timing_model(op: Op) -> String {
    ...
    for instr in op.body {
        ...
        match instr.kind.as_str() {
            ...
            "rep" => {
                // The ext half only widens the base rep's fields; it must not
                // produce a second repetition operator.
                if instr_segments.get_value("ext") == "1" {
                    continue;
                }
...
mod tests {
    ...
    fn test_transit_with_two_repeats() {
        let op = make_op(vec![
            make_instr("evt", vec![("agu_idx", "0"), ("init_addr", "0")]),
            make_instr(
                "rep",
                vec![
                    ("agu_idx", "0"),
            ...
            make_instr("evt", vec![("agu_idx", "1"), ("init_addr", "18")]),
            make_instr(
                "rep",
                vec![
                    ("agu_idx", "1"),
