...
void IOSRAMBottom::setup() {}

void IOSRAMBottom::complete(unsigned int phase) {}

void IOSRAMBottom::finish() { out.verbose(CALL_INFO, 1, 0, "Finishing\n"); }

bool IOSRAMBottom::clockTick(SST::Cycle_t currentCycle) {
  executeScheduledEventsForCycle(currentCycle);
  return false;
}

void IOSRAMBottom::decodeInstr(uint32_t instr) {
...
void IOSRAMBottom::handleRep(uint32_t instr) {
  // Instruction fields
...
void IOSRAMBottom::handleRepx(uint32_t instr) { handleRep(instr); }

void IOSRAMBottom::handleDSU(uint32_t instr) {
  // Instruction fields
  uint32_t slot = getInstrSlot(instr);
...
void IOSRAMBottom::readFromSRAM() {
