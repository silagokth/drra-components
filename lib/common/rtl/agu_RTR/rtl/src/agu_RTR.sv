...
#(
    ...
    // The lane `addr` was generated by. Constant zero where there are no
    // transitions to move between lanes; see mt_ir. Width guarded the way
    // agu_cfg_if guards MT_DIM, since NUMBER_MT may legitimately be zero here.
    output logic [((NUMBER_MT > 0) ? $clog2(NUMBER_MT+1) : 1)-1:0] active_lane
);
  initial begin
