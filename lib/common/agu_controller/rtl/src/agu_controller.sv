...
//
// Used by the io / iosram_top / iosram_btm / iosram_both / rf / dpu resources.
// The only per-resource variation is the EVT port (the iosram split adds a slot
// offset); that is resolved in the resource top and passed in via `evt_port`.
//
...
module agu_controller #(
  ...
  typedef struct packed {
    ...
    // Per-IR-lane initial address, one entry per lane, indexed [0..NUMBER_MT]
    // to match the first dimension of ir_configs. ir.sv offsets each lane's
    // generated address by its own entry, so lanes chained by MT transitions
    // keep separate base addresses.
  ...
  // Lane an EVT opens. current_lane_index is post-incremented on every EVT and
  // resets to '1, so the first EVT wraps it to lane 0 and each chained EVT
  // advances one lane. The lane being opened is therefore the incremented
  ...
  always_ff @(posedge clk or negedge rst_n) begin : ff_agu_config_process
    if (!rst_n) begin
      ...
      if (evt_valid) begin
        ...
        // Default configuration of the lane this EVT opens: emit its initial
        // address once (iter = 1). Indexed by evt_lane rather than lane 0, so a
        // second EVT chained onto the same AGU configures its own lane instead
        // of overwriting lane 0's entry.
        agu_configs_reg[agu_index].ir_configs[evt_lane][0].iter          <= 1;
        agu_configs_reg[agu_index].ir_configs[evt_lane][0].is_configured <= 1'b1;
        // Store the initial address alongside, so each lane keeps its own base.
        agu_configs_reg[agu_index].init_addr[evt_lane]                   <= evt_init_addr;

      end else if (rep_valid) begin

        if (use_or_reg[agu_index]) begin
          if (!rep_ext) begin
            agu_configs_reg[agu_index].or_configs[rep_lvl].delay[DELAY_HI-1:0] <= rep_delay;
            agu_configs_reg[agu_index].or_configs[rep_lvl].iter[ITER_HI-1:0]   <= rep_iter;
            agu_configs_reg[agu_index].or_configs[rep_lvl].step[STEP_HI-1:0]   <= rep_step;
          end else begin
            agu_configs_reg[agu_index].or_configs[rep_lvl].delay[REP_DELAY_WIDTH-1:DELAY_HI] <= rep_delay;
            agu_configs_reg[agu_index].or_configs[rep_lvl].iter[REP_ITER_WIDTH-1:ITER_HI]    <= rep_iter;
            agu_configs_reg[agu_index].or_configs[rep_lvl].step[REP_STEP_WIDTH-1:STEP_HI]    <= rep_step;
          end
          agu_configs_reg[agu_index].or_configs[rep_lvl].is_configured <= 1'b1;
          or_configured_reg[agu_index] <= 1'b1;

        end else begin
          if (!rep_ext) begin
            agu_configs_reg[agu_index].ir_configs[rep_opt][rep_lvl].delay[DELAY_HI-1:0] <= rep_delay;
            agu_configs_reg[agu_index].ir_configs[rep_opt][rep_lvl].iter[ITER_HI-1:0]   <= rep_iter;
            agu_configs_reg[agu_index].ir_configs[rep_opt][rep_lvl].step[STEP_HI-1:0]   <= rep_step;
          end else begin
            agu_configs_reg[agu_index].ir_configs[rep_opt][rep_lvl].delay[REP_DELAY_WIDTH-1:DELAY_HI] <= rep_delay;
            agu_configs_reg[agu_index].ir_configs[rep_opt][rep_lvl].iter[REP_ITER_WIDTH-1:ITER_HI]    <= rep_iter;
            agu_configs_reg[agu_index].ir_configs[rep_opt][rep_lvl].step[REP_STEP_WIDTH-1:STEP_HI]    <= rep_step;
          end
          agu_configs_reg[agu_index].ir_configs[rep_opt][rep_lvl].is_configured <= 1'b1;
        end

        // Only the base REP advances the level; the REPX writes the upper half
        // of the same entry (see rep_lvl above).
        if (!rep_ext)
