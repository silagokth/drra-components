task automatic evaluate_MT_state(
    input logic [$clog2(NUMBER_MT+1)-1:0] level_MT, input logic co_delay_MT[NUMBER_MT],
    output logic en_delay_MT[NUMBER_MT], output logic en_level_MT, output state_t n_state);
...
task automatic evaluate_OR_state(
    input logic co_iter_OR[NUMBER_OR], input logic co_delay_OR[NUMBER_OR],
    input logic regOR_config[NUMBER_OR], output logic en_level_OR,
    output logic [$clog2(NUMBER_OR)-1:0] inVal_level_OR, output logic en_initVal_OR[NUMBER_OR],
    output logic en_delay_OR[NUMBER_OR], output state_t n_state);
  en_level_OR = 1'b0;
  inVal_level_OR = 0;
...
task automatic evaluate_IR_state(
    input logic co_delay_MT[NUMBER_MT], output logic en_delay_MT[NUMBER_MT],
    output logic init0_level_MT, output logic en_level_MT, input logic co_iter_OR[NUMBER_OR],
    input logic co_delay_OR[NUMBER_OR], input logic regOR_config[NUMBER_OR],
    output logic en_level_OR, output logic [$clog2(NUMBER_OR)-1:0] inVal_level_OR,
    output logic en_initVal_OR[NUMBER_OR], output logic en_delay_OR[NUMBER_OR],
    output logic en_flag_OR, input logic [$clog2(NUMBER_MT+1)-1:0] level_MT,
    input logic co_iter_IR[NUMBER_IR], input logic co_delay_IR[NUMBER_IR],
    input logic regIR_config[(NUMBER_MT+1)*NUMBER_IR], output logic en_level_IR,
    output logic [$clog2(NUMBER_IR)-1:0] inVal_level_IR, output logic en_initVal_IR[1:NUMBER_IR-1],
    output logic en_delay_IR[NUMBER_IR], output logic init_delay_IR[NUMBER_IR],
    ...
    if (!co_iter_IR[1] && !co_delay_IR[1]) begin
      ...
      if (!co_iter_IR[2] && !co_delay_IR[2]) begin
        ...
        if (!co_iter_IR[3] && !co_delay_IR[3]) begin
          ...
          en_flag_OR = 1'b1;
          init_delay_IR[0] = 1'b1;
          evaluate_OR_state(co_iter_OR, co_delay_OR, regOR_config, en_level_OR, inVal_level_OR,
                            en_initVal_OR, en_delay_OR, n_state);
        end else begin
          n_state = IDLE;
        end
      end else if (regMT_config[level_MT]) begin
