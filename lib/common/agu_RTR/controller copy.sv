module controller #(
    ...
    for (i = 0; i < NUMBER_IR; i++) begin : IR_counter
        down_counter #(
            ...
            .enable(en_iter_IR[i]),
            .init(init_iter_IR[i]),
            .init_value(init_value_iter_IR[i]-1),
            .co(co_iter_IR[i]),
        ...
        down_counter #(
            ...
            .init_value(init_value_delay_IR[i]),
    ...
    always_comb begin : MUX_INIT_COUNTERS
        for (int i = 0; i < NUMBER_IR; i++) begin
            init_value_iter_IR[i] = regIR_iter[level_MT*NUMBER_IR+i];   //one mux per i
            init_value_delay_IR[i] = regIR_delay[level_MT*NUMBER_IR+i];
        end
    end


    // assign init_value_iter_IR[level_IR]  = regIR_iter[level_MT*NUMBER_IR+level_IR];
    // assign init_value_delay_IR[level_IR] = regIR_delay[level_MT*NUMBER_IR+level_IR];
    // assign init_value_delay_MT[level_MT] = regMT_delay[level_MT];
    // assign init_value_iter_OR[level_MT]  = regOR_iter[level_MT];
    // assign init_value_delay_OR[level_OR] = regOR_delay[level_OR];
    /////////////////////////////////////// FSM PART ///////////////////////////////////////
    ...
    always_comb begin : CONTROLLER_COMB
        ...
        case (p_state)
            ...
            ACTV: begin
                if (!activation) begin 
                    n_state = ACTV;
                end else if (activation && regIR_config[0]) begin
                    n_state = ADDR0;
                end else if (activation && !regIR_config[0]) begin
            ...
            GENR_IR0: begin
                if (!co_delay_IR[0]) begin
                    en_delay_IR[0] = 1'b1;
                    n_state = WAIT_IR0;
                end  else if (!co_iter_IR[0]) begin
                    address_valid = 1'b1;
                    en_address = 1'b1;
                    en_iter_IR[0] = 1'b1;
                    init_delay_IR[0] = 1'b1;
                    ...
                    address_valid = 1'b1;
                    en_address = 1'b1;
                    if (regIR_config[1]) begin
                        if (!co_iter_IR[1]) begin
                            n_state = GENR_IR1;
                            en_level_IR = 1'b1;
                            inVal_level_IR = 1;
                            en_initVal_IR[1] = 1'b1;
                            init_delay_IR[1] = 1'b1;
                        end else if (regIR_config[2]) begin
                            if (!co_iter_IR[2]) begin
                                n_state = GENR_IR1; 
                                en_level_IR = 1'b1;
                                inVal_level_IR = 2;
...
///////////////////////////////////
            GENR_IR1: begin
                if (!co_delay_IR[level_IR]) begin
                    ...
                    n_state = WAIT_IR1;
                end  else if (!co_iter_IR[level_IR]) begin
                    address_valid = 1'b1;
                    init_address = 1'b1;
            ...
            WAIT_IR1: begin
                if (!co_delay_IR[level_IR]) begin
                    en_delay_IR[level_IR] = 1'b1;
                    n_state = WAIT_IR1;
                end else if (co_delay_IR[level_IR]) begin
                    n_state = GENR_IR1;
                end
            end
...
/////////////////////////////////// T-level ///////////////////////////////////    
            GENR_MT: begin
                if (!co_delay_MT[level_MT]) begin
                    ...
                    n_state = GENR_IR0;
                    for (int i = 0; i < NUMBER_IR; i++) begin
                        init_delay_IR[level_MT*NUMBER_IR+i] = 1'b1;
                        init_iter_IR[level_MT*NUMBER_IR+i] = 1'b1;
                    end
                    init0_level_IR = 1'b1;
                end
            end
...
/////////////////////////////////// OR-level ///////////////////////////////////    
            GENR_OR: begin
                if (!co_delay_OR[level_OR]) begin
                    ...
                    n_state = WAIT_OR;
                end  else if (!co_iter_OR[level_OR]) begin
                    address_valid = 1'b1;
                    init_address = 1'b1;
